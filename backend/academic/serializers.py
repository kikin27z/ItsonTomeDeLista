...
class ScheduleSerializer(serializers.Serializer):
  ...
  teacher_id = serializers.IntegerField(write_only=True, allow_null=True, required=False)
  ...
  days_of_week = serializers.RegexField(
    max_length=12,
    regex =r'^(Lu)?(Ma)?(Mi)?(Ju)?(Vi)?(Sa)?$'
  )
  academic_period = serializers.CharField(max_length=40)
  ...
  def update(self, instance, validated_data):
    ...
    instance.academic_period = validated_data.get('academic_period', instance.academic_period)
...
class EnrollmentSerializer(serializers.Serializer):
  ...
  def validate_status(self, value):
    ...
    if value not in valid_values:
      raise serializers.ValidationError("Estado de la asignacion de la inscripcion")
    return value
